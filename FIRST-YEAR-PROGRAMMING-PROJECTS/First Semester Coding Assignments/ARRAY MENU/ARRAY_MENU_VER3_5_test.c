...
int main(){
    int X[5];
    int Y[5];
    int num_count, num_tracker, sort_counter;
    char addnum, try, exit_operation, name[80];
    int i, j;
    int operation, bef_exit;
    int toSearch, found, sorting, temp, toInsert, pos, toDelete;
            ...
            case 1:{ //Searching
                do{
                    ...
                    if(sort_counter == 0){
                        ...
                        printf("}");}
                    else if(sort_counter > 0){
                        for(i = 0; i < 5; i++){
                            printf("%d%s", (Y[i] == -1) ? 0 : Y[i], (i == 4) ? "" : ", ");
                        }
                    }
